#!/usr/bin/env python3
"""Sample scraper: fetch Mercyhurst punt return stats for 2025.

Writes to "Season Stats/Individual/Special Teams/mercyhurst_punt_return.csv".
...
OUT_PATH = os.path.join("Season Stats", "Individual", "Special Teams", "mercyhurst_punt_return.csv")
# Run full range 2012-2025
...
def find_table(soup):
    # prefer explicit punt-return caption
    for table in soup.find_all('table'):
        cap = table.caption.get_text(strip=True) if table.caption else ''
        if 'PUNT' in cap.upper() and 'RETURN' in cap.upper():
            headers = []
    ...
    # fallback: look for tables with NO, YDS, AVG and avoid receiving tables
    for table in soup.find_all('table'):
        headers = []
        thead = table.find('thead')
        if thead:
        ...
        if 'NO' in hdrset and 'YDS' in hdrset and 'AVG' in hdrset and 'TD' in hdrset:
            # avoid tables with REC/RECEPTIONS (receiving)
...
def parse_table(table, headers):
    ...
    for tr in tbody.find_all('tr'):
        ...
        # skip header-like / section rows
        if any(normalize_header(c) in ('PLAYER', 'PUNT RETURN') for c in cols):
            continue
...
def fetch_year(year):
    ...
    if table is None:
        print(f"No punt-return table found for {year}")
        return []
